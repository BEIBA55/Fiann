...
export default function ProfilePage() {
        ...
        <main className="container mx-auto px-4 py-12">
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center">
              ...
              <p className="text-lg text-gray-600 font-medium">Загрузка профиля...</p>
            </div>
          </div>
        </main>
      </div>
  ...
  const handleCancelRegistration = async (registrationId: string, eventTitle: string) => {
    if (!confirm(`Вы уверены, что хотите отменить регистрацию на "${eventTitle}"?`)) {
      return;
    }
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-8 mb-6 border border-gray-100">
            <h1 className="text-4xl font-extrabold text-gray-900 mb-6">Профиль</h1>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-gray-100">
            <h2 className="text-3xl font-bold mb-6">Мои регистрации</h2>
            {registrations.length === 0 ? (
              <p className="text-gray-500">Пока нет регистраций.</p>
            ) : (
